...
func NewTurbineCharacteristic(etaT0, piT0, massRateNorm0, lambdaU0, stageNum float64) TurbineCharacteristic {
...
func (tc *turbineCharacteristic) massRateNorm(piStag float64) float64 {
	return tc.massRateNorm0 * tc.normMassRateRel(tc.x(piStag))
}

func (tc *turbineCharacteristic) x(piStag float64) float64 {
	return (piStag - 1) / (tc.piT0 - 1)
}
...
func (tc *turbineCharacteristic) normMassRateRel(x float64) float64 {
	if x > 1 {
		...
		return math.Pow(1-(1-x)*(1-x), 1/4)
	} else if 0.1 < x && x <= 0.3 {
		return 1.26 * math.Pow(x, 1/3)
	}
	return 1.85 * math.Sqrt(x)
}
...
func (tc *turbineCharacteristic) yStarOptNorm(lambdaU float64) float64 {
	...
	return 0.91 + 0.36*(lambdaUNorm-0.5)*(lambdaUNorm-0.5)
}

func (tc *turbineCharacteristic) yStar(lambdaU, piTStag float64) float64 {
	return math.Sqrt(tc.stageNum) * lambdaU / tc.lambdaAd(piTStag)
}

func (tc *turbineCharacteristic) lambdaAd(piTStag float64) float64 {
	return math.Sqrt((kGas + 1) / (kGas - 1) * (1 - math.Pow(piTStag, -kGas/(kGas-1))))
}

func (tc *turbineCharacteristic) etaTMax(lambdaU float64) float64 {
	return tc.etaT0 * tc.etaTMaxNorm(tc.lambdaUNorm(lambdaU))
}

func (tc *turbineCharacteristic) etaTMaxNorm(lambdaUNorm float64) float64 {
	if lambdaUNorm >= 1 {
		return 1 + 0.03*(lambdaUNorm-1)
	} else if lambdaUNorm > 0.5 {
		return 1 - (0.1+0.18*(math.Pow(tc.piT0, 0.4)-1.44))*(1-lambdaUNorm)
	}
	return 1 - 0.5*(0.1+0.18*(math.Pow(tc.piT0, 0.4)-1.44))
}

func (tc *turbineCharacteristic) lambdaUNorm(lambdaU float64) float64 {
	return lambdaU / tc.lambdaU0
}
