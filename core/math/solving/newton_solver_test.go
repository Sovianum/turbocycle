...
func TestNewtonSolver_Solve_Linear_1d(t *testing.T) {
	var eqSystem = NewEquationSystem(func(x *mat.VecDense) (*mat.VecDense, error) {
		return mat.NewVecDense(1, []float64{x.At(0, 0) - 1}), nil
	}, 1)

	var solver, solverErr = NewUniformNewtonSolver(eqSystem, mat.NewVecDense(1, []float64{0}), 1e-3)
	assert.Nil(t, solverErr)

	var solution, solutionErr = solver.Solve(1e-8, 100)
...
func TestNewtonSolver_Solve_Linear_2d(t *testing.T) {
	...
	var solver, solverErr = NewUniformNewtonSolver(eqSystem, mat.NewVecDense(2, []float64{0, 0}), 1e-3)
	assert.Nil(t, solverErr)
...
func TestNewtonSolver_Solve_NonLinear_2d(t *testing.T) {
	...
	var solver, solverErr = NewUniformNewtonSolver(eqSystem, mat.NewVecDense(2, []float64{-0.5, -0.5}), 1e-3)
	assert.Nil(t, solverErr)
